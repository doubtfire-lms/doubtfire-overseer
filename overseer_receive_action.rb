...
module CONSTANTS
  ...
  # Mounted as a volume
  DOCKER_WORKDIR   = 'work-dir/sandbox'
end
# End ==========================================================
...
def run_assessment_script_via_docker(output_path, random_string, exec_mode, command, image_name_tag)
  ...
  puts 'Running container with the following configuration:'
  puts "  container_name: #{container_name}"
  puts "  image_name_tag: #{image_name_tag}"
  puts '  volumes:'
  puts "    - #{host_exec_path}:/#{CONSTANTS::DOCKER_WORKDIR}"
  ...
  result = {
    ...
    --restart no \
    --cpus 1 \
    --volume #{host_exec_path}:/#{CONSTANTS::DOCKER_WORKDIR} \
...
def cleanup_host_parent_path
  return if host_xfs_volume_path.nil?
  return unless File.exist? host_xfs_volume_path

  puts "Cleaning HOST_XFS_VOLUME force-recursively: #{host_xfs_volume_path}/*"
  FileUtils.rm_rf(Dir.glob("#{host_xfs_volume_path}/*"))
end
...
def receive(subscriber_instance, channel, results_publisher, delivery_info, _properties, params)
  ...
  if File.exist? host_xfs_volume_path
    puts "Skipping cleaning up #{host_xfs_volume_path}"
  ...
  # Step 1
  if valid_zip_file_param? params
    extract_zip submission, host_exec_path
  else
    `cp -R #{submission}/. #{host_exec_path}`
  end

  # Step 2
  extract_zip assessment, host_exec_path
  ...
  run_assessment_script_via_docker(
    ...
    random_string,
    CONSTANTS::BUILD,
    "pwd && ls -la && chmod u+x /#{CONSTANTS::DOCKER_WORKDIR}/#{CONSTANTS::BUILD}.sh && /#{CONSTANTS::DOCKER_WORKDIR}/#{CONSTANTS::BUILD}.sh /#{CONSTANTS::DOCKER_OUTDIR}/#{random_string}.yaml >> /#{CONSTANTS::DOCKER_OUTDIR}/#{random_string}.txt",
  ...
  run_assessment_script_via_docker(
    ...
    random_string,
    CONSTANTS::RUN,
    "chmod u+x /#{CONSTANTS::DOCKER_WORKDIR}/#{CONSTANTS::RUN}.sh && /#{CONSTANTS::DOCKER_WORKDIR}/#{CONSTANTS::RUN}.sh /#{CONSTANTS::DOCKER_OUTDIR}/#{random_string}.yaml >> /#{CONSTANTS::DOCKER_OUTDIR}/#{random_string}.txt",
    docker_image_name_tag
  )
